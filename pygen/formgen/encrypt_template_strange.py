...
class parse_template:
  ...
  def write_new_template(self,out_file):
    ...
    while(own_counter < self.len_analysed_commands):
      ...
      found_name = 0
      if(own_counter == 0):
        files.file_write(out_file,self.template[0:(self.position_signs[0][self.analysed_commands[own_counter][1][0]])])
      else:
        files.file_write(out_file,self.template[(self.position_signs[1][self.analysed_commands[(own_counter-1)][1][1]]+1):(self.position_signs[0][self.analysed_commands[own_counter][1][0]])])
    ...
    files.file_write(out_file,self.template[(self.position_signs[1][self.analysed_commands[(self.len_analysed_commands-1)][1][1]]+1):self.len_template])
    #self.position_signs[1][self.counter]+1    
