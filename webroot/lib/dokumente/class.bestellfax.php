...
class BestellfaxPDF extends Briefpapier {
  public $doctype;

  function BestellfaxPDF($app)
  {
    $this->app=&$app;
    //parent::Briefpapier();
    $this->doctype="rechnung";
    $this->doctypeOrig="Bestellfax";
    parent::Briefpapier(&$this->app);
  }

  function GetBestellfaxPDF($data)
  {
    ...
    if($this->recipient['city']!="")
      ...
      {
        $zahlungsweisetext = "Nachnahme: Beachten Sie das die Post zum Betrag nochmals 2.00 EUR bei Ãergabe des Paketes berechnet.";
      }
	...
	$zahlungsweisetext = "Die Rechnung muss per Vorkasse bezahlt werden.\n\nBankverbindung:\nembedded projects GmbH, Konto 020746400, Deutsche Bank, BLZ 72070001\nIBAN DE75720700010020746400, BIC/SWIFT DEUTDEMM720, Deutsche Bank";
      ...
      $this->setCorrDetails(array("Auftrag"=>$auftrag,"Datum"=>$datum));

      $this->setTextDetails(array(
          "body"=>"Sehr geehrte Damen und Herren,\n\nsenden Sie dieses Bestellfax bitte an die Telefax-Nummer: +(49) 821 279599-20",
  ...
  function SetLieferadresse($data)
  {
      $this->deliverAdress[name]=$data[name];
      $this->deliverAdress[strasse]=$data[strasse];
      $this->deliverAdress[plz]=$data[plz];
      $this->deliverAdress[ort]=$data[ort];
      $this->deliverAdress[land]=$data[land];
