...
class Briefpapier extends SuperFPDF {
  ...
  public function addServiceItem($rdata){
    $this->serviceItems[]=$rdata;
  }
  ...
  function setRecipientLieferadresse($id,$table)
  {
    ...
    $this->recipient['address2']   = $tmp[0][adresszusatz];
  ...
  public function Header() {
    ...
    if(isset($this->logofile) && $this->logofile!="none") 
  ...
  public function Footer() {
    ...
    $this->Cell(0,8,'Seite '.$this->PageNo().' von {nb}',0,0,'L');
  ...
  public function renderDocument() {
    ...
    //$this->AddFont('Arial','B','HLBC____.php');
    //$this->AddFont('HelveticaBoldCondItalic','','HLBCO___.php');
    //$this->AddFont('Arial','','HLC_____.php');
    //$this->AddFont('ArialItalic','','HLCO____.php');
    ...
    if(!empty($this->deliverAdress))   
      $this->renderDeliverAdress();
  ...
  public function renderDeliverAdress()
  {
    $this->SetFont('Arial','B',11);
    $this->Ln(3);
    $this->Cell(80,5,"Lieferadresse:",0,1); 
    
    $this->SetFont('Arial','',10);
    if($this->deliverAdress['name']!="")
      $this->Cell(80,5,$this->deliverAdress['name'],0,1);

    if($this->deliverAdress['strasse']!="")
      $this->Cell(80,5,$this->deliverAdress['strasse'],0,1);
    
    if($this->deliverAdress['plz']!="" && $this->deliverAdress['ort'])
    {
      $this->SetFont('Arial','B',11);
      $this->Cell(80,5,$this->deliverAdress['land']."-".$this->deliverAdress['plz']." ".$this->deliverAdress['ort'],0,1);
    }
  }
  ...
  public function renderRecipient(){
    ...
    {
      $this->Ln(2);
      $this->Cell(80,5,"Telefon: ".$this->recipient['phone'],0,1);
    }
  
    if($this->recipient['email']!="")
      $this->Cell(80,5,"E-Mail: ".$this->recipient['email'],0,1);
...
//    $this->Cell(60,$lineHeight,$date,0,2);
  ...
  public function renderCorrDetails() {
    ...
    if($boldTitleStr!="")
    {
      ...
      $this->SetXY(-38,80);
      $this->MultiCell(28,4,$boldValueStr,"TR",'R');    
    } else {
      $this->SetXY(-75,80);
      $this->MultiCell(37,0,"","TL",'R');
      $this->SetXY(-38,80);
      $this->MultiCell(28,0,"","TR",'R');    
    ...
    $this->SetXY(-38,$tempY);
    $this->MultiCell(28,4,$valueStr,"RB",'R');
  }  
  ...
  public function renderServiceItems()
  {
    $posWidth = 10;
    $itemNoWidth = 40;
    $amountWidth = 25;
    ...
    $this->Cell($posWidth,6,'Pos.',0,0,'C');
    $this->Cell($itemNoWidth,6,'Artikelnr');
    $this->Cell($amountWidth,6,'Anzahl');
    $this->Cell($nameWidth,6,'Beschreibung');
    $this->Ln();
    $this->Line($this->GetX(), $this->GetY(), 190, $this->GetY());
    $this->Ln(2);

    $this->SetFont('Arial','',9);
    for($i=0;$i<count($this->serviceItems);$i++)
    {
      $this->Cell($posWidth,6,$i+1,0,0,'C');
      $this->Cell($itemNoWidth,6,$this->serviceItems[$i][id]);
      $this->Cell($amountWidth,6,$this->serviceItems[$i][amount]);
      $this->Cell($nameWidth,6,$this->serviceItems[$i][name]);
      $this->Ln();
    }
    $this->SetFont('Arial','',10);
    $this->Line($this->GetX(), $this->GetY(), 190, $this->GetY());
  ...
  public function renderNotice()
  {
    $this->SetFont('Arial','B',11);
    $this->Ln(4);
    $this->Cell(20,11,"Bemerkung:");
    $this->Ln();
    $this->SetFont('Arial','',10);
    $this->MultiCell(180,5,str_replace("\\r\\n","\n",$this->notice),1,'L');
  }
