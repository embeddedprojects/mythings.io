...
class ObjGenArbeitspakete
{
  ...
  public function ObjGenArbeitspakete($app)
  {
    $this->app = $app;
  }
  ...
  {
    if(is_numeric($id))
      $result = $this->app->DB->SelectArr("SELECT * FROM arbeitspakete WHERE (id = '$id')");
    else
      return -1;
...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO arbeitspakete (id,adresse,aufgabe,beschreibung,projekt,zeit_geplant,kostenstelle,status,abgabe,abgenommen,abgenommen_von,abgenommen_bemerkung,initiator,art,abgabedatum,logdatei)
  ...
  public function Delete($id="")
  {
    ...
    $sql = "DELETE FROM arbeitspakete WHERE (id='{$this->id}')";
